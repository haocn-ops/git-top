...
Production quality and smoke gates retry transient responses across the canonical
and workers.dev origins while preserving strict D1 and response assertions.
...
GitHub Actions also runs `production-preventive-maintenance` every four hours at minute 25 UTC. It consumes up to 40 structured due items and advances four persisted alternatives batches. When the primary Cron is healthy and nothing is due, the compensation run performs no repository sync. If global sync freshness is stale while every tier queue is still below its overdue threshold, the task performs one lightweight `next_batch` heartbeat sync so a missed Cron window can recover instead of producing a stale-only false failure. The task still fails on sync errors or unhealthy/stale runtime status after recovery; a remaining quality-stale backlog is reported through `outcome.stale_backlog` and the quality risk surfaces without turning a successfully executed compensation batch into a failed governance run. This is a bounded compensation path for missed Worker Cron windows, not a second discovery scheduler.
