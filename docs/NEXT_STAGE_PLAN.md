...
Git.Top is a public V1 baseline for an agent-native GitHub knowledge layer. It has:
...
Status: complete for the public V1 baseline.

Completion notes:

- Production deployment is live at `https://git.top`.
- Production D1 smoke verification passes with `metadata.source: "d1"`.
- The seed baseline has 500 repositories across all 13 V1 categories, including the latest 62-repository live-checked expansion after the 438-repository baseline.
...
Status: complete through the 500-repository V1 growth target.
...
- After live verification cleanup and curated expansion batches, the current seed baseline is 500 repositories and covers all 13 V1 categories.

Current expansion posture:

- The 500-repository V1 growth target is complete.
...
- The latest authenticated expansion live check passed with 62/62 newly added repositories OK.
...
- The current seed list has 500 repositories.
- The latest authenticated expansion live check passed with 62/62 newly added repositories OK.
- The 500-repository growth target is complete; further growth should be treated as post-V1 curation, not default expansion.
...
- Current CI-safe eval baseline is generated in `docs/EVAL_QUALITY.md`: 28 evaluated cases, 504 generated fixture projects, 500 effective generated fixture projects, no synthetic projects, top-1/top-3/category/deployment/Cloudflare-readiness rates all at 1.0, and 0 unacceptable hits.
...
- Current low-confidence review baseline is generated in `docs/LOW_CONFIDENCE_REVIEW.md`: 504 projects reviewed, 0 projects needing review, and 0 actionable low- or medium-confidence review signals.
...
- Current local eval baseline is generated in `docs/EVAL_LOCAL.md`: 23 evaluated cases, 504 generated fixture projects, 500 effective generated fixture projects, no synthetic projects, top-1/top-3/category/deployment/Cloudflare-readiness rates all at 1.0, and 0 unacceptable hits.
...
- Current ranking experiment results are generated in `docs/RANKING_EXPERIMENTS.md`; the runtime baseline is now the strongest overall result, so experimental browse-quality strategies remain offline until they improve local discovery without reducing CI-safe exact-intent quality.
...
- Cloudflare readiness has positive signal and blocker evidence.
- Low-confidence review output is generated by `pnpm quality:review`.
- Classification and Cloudflare readiness are covered by regression validation.
- Reviewed classification overrides can be persisted through the protected `/api/admin/classification-overrides` API and `classification_overrides` D1 table.
...
- `docs/AGENT_QUICKSTART.md`.
- README quickstart link for agent developers.
...
- REST, MCP, and GRP docs include production endpoints and metadata guidance.
- The agent quickstart gives a short path from health check to search, project lookup, recommendation, comparison, MCP, and GRP.
