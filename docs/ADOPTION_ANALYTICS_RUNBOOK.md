...
The report performs fixed-field aggregate Analytics Engine queries for each window: bounded dimension counts, operator-source counts, and agent-call latency quantiles. It defaults to excluding events tagged `source=production-smoke` or `source=operator-check` before aggregation and reports the exclusion count for each window. The production smoke client applies the equivalent `x-git-top-source` header to every request. Known VerifyMCP authentication probe names are normalized to the bounded operation `verification_probe` and automatically tagged `operator-check`, including when older Analytics Engine rows are normalized during reporting. Override the operator-only source list with `--exclude-source production-smoke,operator-check,another-operator-source`. The output includes normalized daily-rate ratios, connect-page views, config copies, successful initializations, first-value calls, workflow completions, tool success, campaign attribution coverage, fallback, strict-source rejection, agent-call latency, bounded client/campaign/operation breakdowns, and a ranked 7-day failure-operation table.
