...
export function renderLlmsTxt(): Response {
  return text(
    [
      ...
      "- Batch project lookup: https://git.top/api/projects?ids=cloudflare/agents,openai/codex&profile=compact",
      "- Project changes: https://git.top/api/changes?limit=50",
      "- Agent feedback validation: https://git.top/api/feedback/proposals",
      "",
      "## Agent Guidance",
      "",
      ...
      "- Use /api/projects for snapshot-consistent batch reads and /api/changes for incremental cache updates and deletion tombstones.",
      "- Continue search, trending, and category lists with page.next_cursor; restart when the snapshot-bound cursor is stale.",
      "- Inspect search.query_interpretation when Git.Top normalizes a known typo or Chinese AI-domain term.",
      "- Use MCP propose_project_feedback to validate evidence-backed corrections; authenticated persistence and administrator review are separate steps.",
...
export function renderLlmsFullTxt(): Response {
  return text(
    [
      ...
      "10. Use /api/projects or MCP get_projects_batch for snapshot-consistent cache hydration, then /api/changes or MCP get_project_changes for incremental updates and deletion tombstones.",
      "11. Cite metadata, classification evidence, quality_signal_confidence, and freshness fields.",
      "12. Operators can use the protected classification override API for reviewed one-off corrections from /quality/review.",
      "",
      "## REST Endpoints",
      "",
      "- GET /api/health",
      "- GET /api/search?q=cloudflare%20agent&limit=5",
      "- GET /api/search?q=langchian&limit=5 (discloses typo normalization and returns snapshot-bound pagination)",
      ...
      "- POST /api/project",
      "- GET/POST /api/projects (1-20 canonical project IDs; compact, decision, or evidence profile)",
      "- GET /api/changes (opaque cursor, 30-day retention, deletion tombstones)",
      "- POST /api/feedback/proposals (public validation; FEEDBACK_SECRET required for persistence)",
